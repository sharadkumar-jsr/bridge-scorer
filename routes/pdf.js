...
async function flexAuth(req, res, next) {
  ...
  try {
    ...
    console.log('[PDF] JWT error:', err.name, err.message);
...
function buildPDF(session, standings, boardRows) {
  const DARK = '#0b2a1a', GOLD = '#c9a03c', LIGHT = '#f5ecda';
  return {
        ...
        body: [
          ...
          ...standings.map((s,i) => {
            ...
            return [
              ...
              {text:s.pairNumber,fillColor:fill},
              {text:pairName(s.player1Name,s.player2Name,s.pairNumber),fillColor:fill},
              {text:s.totalMP,alignment:'right',fillColor:fill},
              {text:s.maxMP,alignment:'right',fillColor:fill},
              {text:`${s.percentage}%`,alignment:'right',bold:true,fillColor:fill,
               color:parseFloat(s.percentage)>=55?'#1a7a3a':parseFloat(s.percentage)<45?'#cc3333':'#000'},
            ];
          }),
        ]}, margin:[0,0,0,20] },
      { text:'Board by Board Results', fontSize:14, bold:true, margin:[0,0,0,8], pageBreak:'before' },
      { table: { headerRows:1, widths:[25,30,45,45,55,40,40],
        body: [
          [
          ...
          ...boardRows.map((r,i) => {
            const fill = i%2===0?'#f0f0e8':'#ffffff';
            const sc = r.ns_score>0?'#1a7a3a':r.ns_score<0?'#cc3333':'#555';
            return [
              ...
              {text:r.ns_score!=null?(r.ns_score>0?`+${r.ns_score}`:r.ns_score):'—',
               alignment:'right',color:sc,bold:true,fillColor:fill},
            ];
          }),
        ]} },
