...
// GET /api/sessions/:id/pdf  (director auth OR released results + player auth)
...
// Middleware: allow director OR player (if results released)
async function flexAuth(req, res, next) {
  ...
  try {
    ...
    if (payload.role === 'player') {
      // Player can only access if results are released
      const { rows } = await db.query(
        `SELECT results_released FROM sessions WHERE id = $1`, [req.params.id]
      );
...
function buildPDF(session, standings, boardRows) {
  ...
  return {
    ...
    content: [
      // ── Header ──────────────────────────────────────────────
      {
        canvas: [{ type: 'rect', x: 0, y: 0, w: 515, h: 70, color: DARK }],
        margin: [0, 0, 0, 0],
      },
      {
        text:     '♠ ♥ ♦ ♣  Bridge Club Scorer',
        ...
        table: {
          ...
          body: [
            ...
            ...standings.map((s, i) => [
              ...
              { text: pairName(s.player1Name, s.player2Name, s.pairNumber), fillColor: i % 2 === 0 ? '#f0f0e8' : '#ffffff' },
          ...
          body: [
            ...
            ...boardRows.map((r, i) => {
              ...
              return [
                ...
                { text: contractStr(r), fillColor: fill, font: 'Helvetica' },
